...
const warmCardNotes = [
  'Ты рядом и день становится мягче',
...
<template>
  <main class="relative min-h-screen px-3 pb-14 pt-12 sm:px-6 sm:pt-16">
    <section class="mx-auto w-full max-w-4xl space-y-4 sm:space-y-6">
      <header class="relative flex flex-col overflow-hidden rounded-2xl border border-stone-200 bg-gradient-to-b from-white to-rose-50/40 p-4 shadow-[0_10px_28px_rgba(0,0,0,0.05)] sm:p-8">
        <span
          aria-hidden="true"
          class="pointer-events-none absolute -right-4 -top-5 text-8xl leading-none text-rose-200/55 sm:text-9xl"
        >
          ❤
        </span>
        <span
          aria-hidden="true"
          class="pointer-events-none absolute -left-4 -bottom-7 text-6xl leading-none text-rose-100/70 sm:text-7xl"
        >
          ❤
        </span>
        ...
        <div class="mt-3 flex flex-wrap gap-1.5 sm:mt-4 sm:gap-2">
          <span
            ...
            :key="word"
            class="inline-flex items-center rounded-full border border-rose-200 bg-white/85 px-3 py-1 text-xs font-medium text-rose-500"
          >
            ✦ {{ word }}
          </span>
        </div>
      ...
      <section class="grid gap-4 sm:grid-cols-3">
        ...
        >
          ...
          />
          <div class="relative flex-1 overflow-hidden bg-gradient-to-b from-white to-rose-50/45 p-3 sm:p-5">
            <span
              aria-hidden="true"
              class="pointer-events-none absolute -right-1 -top-3 text-5xl leading-none text-rose-200/55"
            >
              ❤
            </span>
            ...
            <div class="mt-3 inline-flex items-center gap-1.5 text-xs text-rose-400">
              <span aria-hidden="true">❤</span>
              <span>{{ warmCardNotes[index] }}</span>
            </div>
          </div>
        </article>
      </section>
